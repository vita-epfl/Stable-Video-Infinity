...
class SVIVideoPipeline(BasePipeline):

    def __init__(self, device="cuda", torch_dtype=torch.float16, tokenizer_path=None, is_test=False, num_train_timesteps=1000):
        super().__init__(device=device, torch_dtype=torch_dtype)
        self.scheduler = FlowMatchScheduler(shift=5, sigma_min=0.0, extra_one_step=True, num_train_timesteps=num_train_timesteps)
    ...
    @staticmethod
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False, is_test=False, num_train_timesteps=1000):
        if device is None: device = model_manager.device
        if torch_dtype is None: torch_dtype = model_manager.torch_dtype
        pipe = SVIVideoPipeline(device=device, torch_dtype=torch_dtype, is_test=is_test, num_train_timesteps=num_train_timesteps)
    ...
    def _sample_with_regular_video(self, latents, prompt_emb_posi, prompt_emb_nega, image_emb, extra_input, tea_cache_posi, tea_cache_nega, usp_kwargs, use_controlnet, cfg_scale, progress_bar_cmd):
        """
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if cfg_scale['text'] != 1.0:
                ...
                noise_pred_cond = model_fn_wan_video(self.dit, latents, timestep=timestep, **prompt_emb_posi, **image_emb, **extra_input, **tea_cache_posi, **usp_kwargs, use_controlnet=use_controlnet)
                ...
                noise_pred_uncond = model_fn_wan_video(
                    ...
                    **usp_kwargs, use_controlnet=use_controlnet, 
                )
    ...
    def __call__(
        ...
        latents = self._sample_with_regular_video(
            latents, prompt_emb_posi, prompt_emb_nega, image_emb, extra_input,
            tea_cache_posi, tea_cache_nega, usp_kwargs, use_controlnet, cfg_scale, progress_bar_cmd
        )
        # Decode
        self.load_models_to_device(['vae'])
        ...
        if hasattr(args, 'sequential_cfg') and args.sequential_cfg == 'latent':
